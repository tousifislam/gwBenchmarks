...
def generate_changelog_entry(scorecard):
    ...
    method_name = model_name.replace('_', ' ').replace('Fit', ' Fit').replace('NNInterp', 'Nearest Neighbor Interpolation')
    
    return f"""
## [V-{approach_number:02d}] {model_name} ({parameterization} parameterization)
- **Time**: {time.strftime('%Y-%m-%d %H:%M', time.localtime())}
- **Benchmark**: {BENCHMARK_NAME}
- **Method**: {method_name}
- **Parameterization**: {parameterization}
- **Loss**: {loss:.4f} (RMSE on log10(mismatch))
...
def update_full_changelog():
    ...
    for scorecard in model_changelog_entries:
        # We will add PySR/gplearn blocked entry manually. Need to ensure its approach_number does not conflict.
        # PhenomFit_Poly_EtaChiEff already acts as the alternative
    ...
    # If PySR/gplearn is truly blocked and PhenomFit is the alternative, add a separate blocked entry for documentation
    # We don't add generate_blocked_pysr_gplearn_entry() because PhenomFit already served as its replacement.
