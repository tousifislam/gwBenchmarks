...
def load_scores(benchmarks_dir: Path) -> list[dict]:
    rows = []
    for score_path in sorted(benchmarks_dir.glob("*/no_skills/score_level13.json")):
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=Path("benchmarks/rg_waveform_benchmark/rg_waveform_all_models_violin"),
    ...
    for item in rows:
        print(
            ...
            f"mean={item['mean']:.6g}, n={item['mismatches'].size}, failed={item['n_failed']}"
        )
